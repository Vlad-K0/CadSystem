...
public class App extends Application {
    ...
    public void start(Stage stage) {
        ...
        rotateLeftBtn.setOnAction(e -> {
            matrixTransform.rotate(-Math.toRadians(15), canvas.getWidth() / 2, canvas.getHeight() / 2);
            redraw();
        });
        ...
        canvas.setOnKeyPressed(event -> {
            if (event.getCode() == KeyCode.LEFT) {
                matrixTransform.rotate(-Math.toRadians(event.isShiftDown() ? 90 : 15), canvas.getWidth() / 2, canvas.getHeight() / 2);
                redraw();
